...
let outs = [];
let package_json = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'));
...
module.exports = function *() {
  ...
  function getAllFiles(root) {
    ...
    files.forEach(function (filename) {
      ...
      let prex = pathname.split(config.appFold)[1].replace(new RegExp('^\\' + path.sep), '');
      ...
      if (stat.isDirectory()) {
        ...
        if (filename.indexOf('.') > 0) {
          ...
          if(extname === '.png' && /tileset/.test(prex)) {
            return true;
          }
          ...
          if(package_json.tplType === 'offlineTpl') {
            prex = '/' + prex;
          }
  ...
  function generateFile() {
    ...
    let prefix = package_json.tplType === 'offlineTpl' && 'module.exports' || 'var RESOURCE';
    fs.writeFile(resourceFilePath, `${prefix} = {${outs.join(',\n')}\n};\n`, function (err) {
      if (err) throw err;
