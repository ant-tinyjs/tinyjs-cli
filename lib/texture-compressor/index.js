...
module.exports = function*($$, program) {
  ...
  const folds = program.parent.args.filter((v) => typeof v === 'string');
  const inputs = [];
  const inputQuality = program.quality;
  let cts;
  ...
  inputs.forEach(function(opts) {
    ...
    ['astc', 'pvr'].forEach(function(type) {
      ...
      const { compression, quality } = constants[inputQuality][type];
      const flagMapping = [
        '-i',
        input,
        '-o',
      ...
      if (fs.existsSync(output)) {
        ...
        utils.hint('info', `${type.toUpperCase()} 格式压缩纹理信息：${output} - ${outFileSize / 1000}KB`);
      }
