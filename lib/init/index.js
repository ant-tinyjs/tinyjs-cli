...
module.exports = function*() {
  ...
  const checkDir = () => {
    ...
    let files = fs.readdirSync(destDir).filter(function (dir) {
  ...
  // 1. 获取问询值
  const tplAnswers = yield require('./query')();
  const {tplType} = tplAnswers;

  if(tplType === 'offlineTpl' || tplType === 'basicTpl') {
    const answers = yield require('./query')(tplAnswers);
    ...
    // 3. 生成项目非在线包
    require('./generate')(Object.assign({}, tplAnswers, answers));
    ...
    try {
      ...
      let ret;
      ret = shell.exec(`npm install`, {silent: false});
    ...
    //生成在线包
    checkDir() && require('./generate')(tplAnswers);
  }
};
