...
module.exports = function*() {
  ...
  try {
    ...
    let ret;
    if (baseName === name) {
      ret = shell.exec(`tnpm install`, {silent: false});
    } else {
      ret = shell.exec(`cd ${name} && tnpm install`, {silent: false});
    }
    if (ret.code === 0) {
    } else {
      ifInstallOk = false;
  ...
  if (ifInstallOk) {
    ...
    utils.hint('success', `Tiny.js 项目 ${chalk.cyan.bold(name)} 初始化完成。`);
    utils.hint('warn', `但是, 依赖安装失败, 请自行执行 ${chalk.green.bold('tnpm install')} 安装依赖。`);
  }
};
